import math
import numpy as np
import behaviour_based_navigation_challenge3 as bn
...
def scan_world(robot, allobstacles, alltargets):
    ...
    final_orientation = 0.0 # Desired final orientation of the robot
    
    # turn_rate = bn.compute_turnrate(target_distance, target_angle_robot, sonar_left, sonar_right)
    turn_rate = bn.compute_turnrate(target_distance, target_angle_robot, sonar_left, sonar_right, final_orientation, robot.phi) # Added extra parameters final_orientation robot.phi
    velocity = bn.compute_velocity(sonar_left, sonar_right, final_orientation, robot.phi) # Added extra parameters final_orientation and robot.phi
